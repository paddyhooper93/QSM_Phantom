...
% output_dir = 'C:\vnm\DRO_220726\Voxels\';
% output_voxel_file = 'Voxels_298by298by210_5mm_air.nii.nii';
% 
% % Prepare geometry from CAD file for segmentation
% 
% polygons = stlread(strcat(input_dir,input_polygon_file));
% voxels = polygon2voxel(polygons,[298 298 210], 'auto');
% % voxels(299:end,:,:) = [];
% % voxels(:,299:end,:) = [];
% % voxels(:,:,308:end) = [];
% % voxels(:,:,1:97) = [];
% 
% voxels_nii = make_nii(double(voxels));
...
segment_dir = 'C:\vnm\7T_RR05_VSHARP_Star\Segmented_Geom\';
input_segmentation_file = 'segmentation_24Labels_7T.nii.gz';

% load segmentated geometry
geom_oriented = niftiread(strcat(segment_dir,input_segmentation_file));

GT_dir = 'C:\vnm\7T_RR05_VSHARP_Star\GT\';
chi_file = 'DRO_Susceptibility_GT.nii.gz';
% DRO susceptibility GT values
N = size(geom_oriented);
chi = zeros(N);

chi(geom_oriented==00) = 0; % Bulk medium
chi(geom_oriented==01) = 0; % Vial wall
...
% 7T_Experimental_Results
for i = 1:5
    chi(geom_oriented==[i+2]) = 13.6*usp(i) + 0.2; 
end

for i = 1:5
    chi(geom_oriented==[i+7]) = 1.81*fer(i) + 0.1; 
end
...
for i = 1:5
    chi(geom_oriented==[i+17]) = -0.01*cal(i) + 0.2;
end
...
segment_dir = 'C:\vnm\7T_RR05_VSHARP_Star\Segmented_Geom\';
segment_file = 'segmentation_24Labels_7T.nii.gz';
geom = niftiread(strcat(segment_dir,segment_file));
geom_trunc = geom(:,:,:);
N = size(geom_trunc);
magn = zeros([N]);
magn_use = zeros([N 9]);
te = [3.15:3.15:28.35]/1000; % secs
working_dir = 'C:\vnm\7T_RR05_VSHARP_Star\7T_Simulated_Recon\';
mask = double(niftiread(strcat(working_dir,"mask.nii")));
output_magn_file = 'DRO_Magn_use.nii.gz';

for i = 1:9
    magn(geom_trunc==00) = 0.5; % SNR_vial_wall = 5/0.01 = 50
    magn(geom_trunc==01) = 1; % SNR_bulk = 10/0.01 = 100
    ...
    for j = 1:5
    magn(geom_trunc==[2+j]) = exp(-te(i)*(1174*usp(j)+17));
    end

    for j = 1:5
    magn(geom_trunc==[7+j]) = exp(-te(i)*(49.9*fer(j)+9));
    end
    
    for j = 1:5
    magn(geom_trunc==[12+j]) = exp(-te(i)*(0.2*cal(j)+7));
    end

    for j = 1:5
    magn(geom_trunc==[17+j]) = exp(-te(i)*(5.0*cal(j)+103));
    end
    
    magn_use(:,:,:,i) = magn;
end

% magn_rep = repmat(magn_cat,[1 1 100 1]);
...
input_dir = 'C:\vnm\7T_RR05_VSHARP_Star\GT\';
working_dir = 'C:\vnm\7T_RR05_VSHARP_Star\7T_Simulated_Recon\';
magn = double(niftiread(strcat(working_dir,'DRO_Magn_use.nii.gz')));
mask = double(niftiread(strcat(working_dir,"mask.nii")));
fm_freq = double(niftiread(strcat(input_dir,'DRO_fm_freq.nii.gz'))); % % Continuous_dipole_kernel.py (Ref: Salomir, Bowtell and Marques (2003))
N = size(fm_freq);

phs_prefix = 'DRO_Phs_use.nii.gz';
