...
%% 3 T
usp_3T = csvread('USPIO_QSM_3T.csv',2,4);
fer_3T = csvread('Ferritin_QSM_3T.csv',2,4);
chl_3T = csvread('CaCl2_QSM_3T.csv',2,4);
crb_3T = csvread('CaCO3_QSM_3T.csv',2,4);


%% Test-retest (intra-session rep)
y3_usp_test = usp_3T(:,1);
y3_fer_test = fer_3T(:,1);
y3_chl_test = chl_3T(:,1);
y3_crb_test = crb_3T(:,5);

y3_usp_retest = usp_3T(:,3);
y3_fer_retest = fer_3T(:,3);
y3_chl_retest = chl_3T(:,3);
y3_crb_retest = crb_3T(:,7);
...
% a_std_pairs = std(data).*1000; % in units ppb
...
%% Time-point stability (inter-session rep)
...
usp_7T = csvread('USPIO_QSM_7T.csv',2,4);
fer_7T = csvread('Ferritin_QSM_7T.csv',2,4);
chl_7T = csvread('CaCl2_QSM_7T.csv',2,4);
crb_7T = csvread('CaCO3_QSM_7T.csv',2,4);
...
a_std_pairs = std(data)./mean(data).* 100; %(in units %)
a_median_sd = median(a_std_pairs);
a_min_sd = min(a_std_pairs);
a_max_sd = max(a_std_pairs);
a_RC_abs = rms(a_std_pairs)*sqrt(2)*1.96; % Absolute coefficient of repeatability
